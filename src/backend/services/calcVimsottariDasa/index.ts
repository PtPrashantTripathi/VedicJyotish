...
import { tropicalYearExtended } from "src/backend/services/calcVimsottariDasa/MeanTropicalYear";
import type { Nakshatra } from "src/backend/services/constants/Nakshatra";
import type { NavagrahaEn } from "src/backend/services/constants/Planet";
import { getPhalDb } from "src/backend/services/phalLoader";
import type { SourceBookEn, Translation } from "src/backend/services/types";
...
function computeDasha(
    ...
    return sequence.map(currentLord => {
        ...
        const dashaEntry: Dasha = {
            ...
            Phal:
                dashaName === "MahaDasha"
                    ? (getPhalDb()?.maha_dasha_phal?.[currentLord] ?? {})
                    : dashaName === "AntarDasha"
                      ? (getPhalDb()?.antar_dasha_phal?.[parentLord]?.[
                            currentLord
                        ] ?? {})
                      : {},
