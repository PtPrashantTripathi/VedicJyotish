class TimeControl {
    constructor(initialDate, onDateChange) {
        ...
        this.btnGotoKaliyugStart = document.getElementById('goto-kaliyug-start');
    ...
    attachEventListeners() {
        // Input fields
        this.yearInput.addEventListener('change', () => this.navigateDate('year', this.yearInput.value));
        this.monthInput.addEventListener('change', () => this.navigateDate('month', this.monthInput.value));
        this.dayInput.addEventListener('change', () => this.navigateDate('day', this.dayInput.value));
        this.hourInput.addEventListener('change', () => this.navigateDate('hour', this.hourInput.value));
        this.minuteInput.addEventListener('change', () => this.navigateDate('minute', this.minuteInput.value));

        // Buttons
        this.btnGotoNow.addEventListener('click', () => this.navigateDate('now'));
        this.btnGotoKaliyugStart.addEventListener('click', () => this.navigateDate('kaliyugStart'));

        // Scrubber
        this.scrubber.addEventListener('mousedown', this.startScrub.bind(this));
        this.scrubber.addEventListener('touchstart', this.startScrub.bind(this), { passive: false });
        ...
        switch (unit) {
            ...
            case 'year':
                ...
                if (!isNaN(yearValue)) {
                    ...
                    alert("Invalid year entered. Please use numbers (e.g., 2023, -3101 for 3102 BCE).");
                    return;
                }
                break;
            case 'hour':
                ...
                } else { console.warn(`Invalid hour: ${value}`); return; }
                break;
            case 'minute':
                const minuteValue = parseInt(value, 10);
                if (minuteValue >= 0 && minuteValue <= 59) {
                    currentDate.setMinutes(minuteValue);
                } else { console.warn(`Invalid minute: ${value}`); return; }
                break;
            case 'now':
    ...
    updateIndicatorPosition() {
        ...
        const clampedDeltaX = Math.max(-maxDelta, Math.min(maxDelta, this.currentDeltaX));
    ...
    startScrub(event) {
        ...
        this.scrubIntervalId = setInterval(this.applyTimeScrub.bind(this), this.scrubIntervalDuration);
        ...
        window.addEventListener('touchmove', this.boundHandleScrubMove, { passive: false });
    ...
    applyTimeScrub() {
        ...
        const msIncrement = (hoursPerSecond * 3600 * 1000) * (this.scrubIntervalDuration / 1000);
