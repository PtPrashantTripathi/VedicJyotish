class SolarSystemSVG {
    ...
    constructor(svgId, config = {}) {
        ...
        if (config.planetConfig) {
            this.config.planetConfig = { ...defaults.planetConfig, ...config.planetConfig };
        }
        if (config.orbitRadiiConfig) {
            this.config.orbitRadiiConfig = { ...defaults.orbitRadiiConfig, ...config.orbitRadiiConfig };
        }
        ...
        this.maxRadius = Math.min(this.center.x, this.center.y) * this.config.maxOrbitRadiusScale;
    ...
    _describeArc(radius, startAngle, endAngle, sweepFlag = 1, centerPoint = this.center) {
        const startCoords = this._getCoordinates(radius, startAngle, centerPoint);
    ...
    _drawSun(sunScreenX, sunScreenY) { // Accept screen coordinates
        ...
        this.svg.appendChild(this._createSVGElement('text', {
            x: this.center.x,
            y: this.center.y,
            class: 'sun-label',
            'text-anchor': 'middle',
    ...
    _drawOrbits(planetsData, centerOfHeliocentricOrbits) {
        const orbitGroup = this._createSVGElement('g', { class: 'orbit-group' });
        ...
        const planetsWithOrbits = planetsData
            .map(p => p.id)
            .filter(id => this.config.orbitRadiiConfig[id] && !this.config.planetConfig[id]?.isSun);
        ...
        sortedPlanetIds.forEach(planetId => {
            ...
            if (radius > 0) {
                orbitGroup.appendChild(this._createSVGElement('circle', {
    ...
    _drawPlanets(planetsData, actualSunScreenCoords, actualEarthScreenCoords) {
        const planetGroup = this._createSVGElement('g', { class: 'planet-group' });
        // Insert moon orbit group *before* planets for better layering
        const moonOrbitGroup = this._createSVGElement('g', { class: 'moon-orbit-group' });
        ...
        sortedPlanets.forEach(planet => {
            ...
            if (planet.id === 'moon') {
                if (actualEarthScreenCoords && this.config.moonOrbitRadiusAroundEarth > 0) {
                    // 1. Draw Moon's orbit around Earth's calculated position
                    moonOrbitGroup.appendChild(this._createSVGElement('circle', {
                    ...
                    console.warn(`Cannot draw Moon: Earth screen position (${actualEarthScreenCoords}) or moonOrbitRadiusAroundEarth (${this.config.moonOrbitRadiusAroundEarth}) invalid.`);
            ...
            // --- End Special Handling for Moon ---
                ...
                if (!positionRadius) { // Check if radius is defined
                    console.warn(`Orbit radius not defined for ${planet.id}, cannot draw.`);
                    return;
                }
                // Planets (other than Moon) are always positioned heliocentrically relative to the Sun's screen position.
                coords = this._getCoordinates(positionRadius, planet.positionAngle, actualSunScreenCoords);
            ...
            if (planet.id === 'earth') {
                ...
                group.addEventListener('click', () => {
                    this.currentViewCenterTarget = (this.currentViewCenterTarget === 'sun') ? 'earth' : 'sun';
                    this.drawSystem(this.rawPlanetsData, this.ayanamsa); // Redraw with new view
                });
            } else if (groupClass.includes('planet-clickable')) { // Moon and other planets for selection
                 group.addEventListener('click', () => {
                    if (this.selectedPlanetId === planet.id) this.selectPlanet(null);
                    else this.selectPlanet(planet.id);
                });
            } // Sun's clickability is handled by its 'planet-clickable' class on its body in _drawSun

            group.appendChild(this._createSVGElement('circle', {
                cx: coords.x,
                cy: coords.y,
                r: planetRadius,
            ...
            })).textContent = planet.retrograde?planet.name+"*":planet.name || '?';
    ...
    _drawRashiChakra() {
        const zodiacGroup = this._createSVGElement('g', { class: 'zodiac-ring-group' });
        ...
        for (let i = 0; i < 12; i++) {
            ...
            // --- Divider Line ---
            const startCoordsDiv = this._getCoordinates(this.config.zodiacRingInnerRadius, startAngle,this.earthCoords);
            const endCoordsDiv = this._getCoordinates(this.config.zodiacRingOuterRadius, startAngle,this.earthCoords);
            zodiacGroup.appendChild(this._createSVGElement('line', {
                x1: startCoordsDiv.x, y1: startCoordsDiv.y,
                x2: endCoordsDiv.x, y2: endCoordsDiv.y,
                class: 'rashi-divider',
            }));
            ...
            const pathData = this._describeArc(this.config.zodiacTextRadius, arcStartAngle, arcEndAngle, sweepFlag,this.earthCoords);
            const pathId = `solarsystem-rashi-path-${i}`; // Unique ID
            defs.appendChild(this._createSVGElement('path', {
                id: pathId,
                d: pathData,
            ...
            const rashiName = typeof rashiNames !== 'undefined' ? rashiNames[i] : '?';
        ...
        zodiacGroup.appendChild(this._createSVGElement('circle', {
    ...
    _removeSelectionLines() {
        if (this.selectionLinePlanet && this.selectionLinePlanet.parentNode === this.svg) {
            this.svg.removeChild(this.selectionLinePlanet);
        }
        this.selectionLinePlanet = null;

        if (this.selectionLineSun && this.selectionLineSun.parentNode === this.svg) {
    ...
    _removeAngleText() {
        if (this.angleTextElement && this.angleTextElement.parentNode === this.svg) {
    ...
    _drawSelectionLines(selectedPlanetData) {
        if (!this.earthCoords || !this.sunScreenCoords) {
            console.warn("Cannot draw selection lines: Earth coordinates not available.");
            return;
        }
        if (!selectedPlanetData) {
             console.warn("Cannot draw selection lines: Selected planet data not available.");
            return;
        }
        ...
        const planetData = this.rawPlanetsData.find(p => p.id === selectedPlanetData.id);
        ...
        if (selectedPlanetData.id === 'moon') {
            ...
            if (this.config.moonOrbitRadiusAroundEarth > 0) {
                ...
                console.warn("Cannot draw selection line to Moon: moonOrbitRadiusAroundEarth is invalid.");
                return;
            }
        } else {
            // Other planets (including Sun if selected)
            const positionRadius = this.config.orbitRadiiConfig[selectedPlanetData.id];
            if (selectedPlanetData.id === 'sun') {
                planetCoords = this.sunScreenCoords; // Sun is at its screen position
            } else if (!positionRadius) {
                 console.warn(`Cannot draw selection line to ${selectedPlanetData.id}: Orbit radius not defined.`);
                 return;
            } else {
                // Other planets are positioned heliocentrically relative to the Sun's screen position
                planetCoords = this._getCoordinates(positionRadius, planetData.positionAngle, this.sunScreenCoords);
            }
        }

        if (!planetCoords) {
             console.warn(`Cannot draw selection line to ${selectedPlanetData.id}: Could not determine coordinates.`);
             return;
        }
        ...
        if (len === 0) { // Avoid division by zero if Earth and planet overlap
             console.warn("Cannot draw Earth-Planet line: Earth and selected planet are at the same coordinates.");
             // Still draw Earth-Sun line if possible
             if(this.selectionLineSun) this.svg.insertBefore(this.selectionLineSun, this.svg.querySelector('.planet-group'));
             return;
        }
    ...
    _drawAngleText(selectedPlanetData) {
        if (!this.earthCoords) {
            console.warn("Cannot draw angle text: Earth coordinates not available.");
            return;
        }
        if (!selectedPlanetData || typeof selectedPlanetData.sunEarthObjectAngle !== 'number' || isNaN(selectedPlanetData.sunEarthObjectAngle)) {
            console.warn("Cannot draw angle text: Selected planet data or angle is missing/invalid.", selectedPlanetData);
            return;
        }
     ...
     * @param {Array<object>} planetsData - Array of planet objects, each with 'id', 'longitude' (geo), and 'positionAngle' (helio).
     *                                      Example: [{ id: 'mars', longitude: 123.45, positionAngle: 110.0 }, ...]
     */
    drawSystem(planetsData,ayanamsa) {
        ...
        if (this.currentViewCenterTarget === 'sun') {
            ...
            if (earthData && this.config.orbitRadiiConfig['earth']) {
                ...
                calculatedEarthScreenCoords = { x: this.center.x - (this.config.orbitRadiiConfig.earth || 100), y: this.center.y }; // Fallback
                console.warn("Earth data/orbit missing for heliocentric view coord calculation.");
            }
        } else { // currentViewCenterTarget === 'earth'
            calculatedEarthScreenCoords = this.center; // Earth is at SVG center

            const earthDataForSunCalc = this.rawPlanetsData.find(p => p.id === 'earth');
            if (earthDataForSunCalc && this.config.orbitRadiiConfig['earth']) {
                ...
                const sunGeoAngle = (earthDataForSunCalc.positionAngle + 180) % 360;
                ...
                calculatedSunScreenCoords = { x: this.center.x + (this.config.orbitRadiiConfig.earth || 100), y: this.center.y }; // Fallback
                console.warn("Earth data/orbit missing for Sun's geocentric coord calculation.");
            }
        }
        ...
        this._drawPlanets(this.rawPlanetsData, this.sunScreenCoords, this.earthCoords);
     ...
     * @param {string | null} planetId - The ID of the planet (e.g., 'mars', 'moon') or null to deselect.
     * @returns {object | null} The selected planet data object or null if deselected/not found/invalid.
     */
    selectPlanet(planetId) {
        ...
        const selectedPlanetData = this.rawPlanetsData.find(p => p.id === planetId);
        ...
        const hasAngle = (isMoon && !isNaN(selectedPlanetData.longitude)) ||
                         (selectedPlanetData.id === 'sun') ||
                         (!isMoon && selectedPlanetData.id !== 'sun' && !isNaN(selectedPlanetData.positionAngle));

        if (!hasAngle) {
             console.warn(`Cannot select planet "${planetId}": Required angle (longitude for Moon, positionAngle for others) is missing or invalid.`);
             this.selectedPlanetId = null;
             return null;
        }
