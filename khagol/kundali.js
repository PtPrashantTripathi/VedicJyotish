 ...
 * This version aims for a more traditional visual appearance.
 */
class KundaliSVG {
    /**
    ...
    _calculateHousePoints() {
        ...
        const innerDiamond = [
            { x: (center.x + outerSquare[1].x) / 2, y: (center.y + outerSquare[1].y) / 2 }, // Top-Right corner (of house 1)
            { x: (center.x + outerSquare[2].x) / 2, y: (center.y + outerSquare[2].y) / 2 }, // Bottom-Right corner (of house 4)
            { x: (center.x + outerSquare[3].x) / 2, y: (center.y + outerSquare[3].y) / 2 }, // Bottom-Left corner (of house 7)
            { x: (center.x + outerSquare[0].x) / 2, y: (center.y + outerSquare[0].y) / 2 }  // Top-Left corner (of house 10)
        ];
    ...
    _getHouseAnchorPoints(houseNumber) {
        ...
        const { outerSquare, center, innerDiamond, outerDiamond } = this.housePoints;
        ...
        switch (houseNumber) {
            ...
            case 1:
                points.rashiLabelPos = { x: center.x, y: outerDiamond[0].y + labelOffset }; // Top point, offset down
                points.planetCenter = { x: center.x, y: (outerDiamond[0].y + innerDiamond[3].y ) / 2 + planetCenterOffsetV };
                points.ascPos = { x: center.x, y: points.rashiLabelPos.y + ascOffset }; // Below Rashi label
                break;
            // Top-left triangle (House 2)
            case 2:
                points.rashiLabelPos = { x: innerDiamond[3].x , y: outerDiamond[0].y + labelOffset*0.7 };
                points.planetCenter = { x: innerDiamond[3].x, y: (outerDiamond[0].y + innerDiamond[3].y) / 2 };
                break;          
            // Left-top triangle (House 3)
            case 3:
                points.rashiLabelPos = { x: outerDiamond[3].x+ labelOffset * 0.7, y:(outerDiamond[0].y  + innerDiamond[0].y) / 2};
                points.planetCenter = { x: (outerDiamond[3].x + innerDiamond[3].x) / 2 - planetCenterOffsetH, y: (outerDiamond[3].y + outerDiamond[0].y + innerDiamond[3].y) / 3 };
                break;
            // Left triangle (House 4)
            case 4:
                points.rashiLabelPos = { x: outerDiamond[3].x + labelOffset, y: center.y  };
                points.planetCenter = { x: (outerDiamond[3].x + innerDiamond[2].x + innerDiamond[3].x) / 3 + planetCenterOffsetH, y: center.y  };
                break;
            // Left-bottom triangle (House 5)
            case 5:
                points.rashiLabelPos = { x: outerDiamond[3].x+ labelOffset * 0.7, y: (outerDiamond[2].y+innerDiamond[3].y)/2};
                points.planetCenter = { x: (outerDiamond[3].x + innerDiamond[3].x) / 2 - planetCenterOffsetH, y: (outerDiamond[1].y + outerDiamond[2].y) / 2  };
                break;
            // Bottom-left triangle (House 6)
            case 6:
                points.rashiLabelPos = { x:innerDiamond[3].x, y:outerDiamond[2].y - labelOffset *0.5 };
                points.planetCenter = { x: (outerDiamond[2].x + outerDiamond[3].x + innerDiamond[2].x) / 3 - planetCenterOffsetH, y: (outerDiamond[2].y + innerDiamond[2].y) / 2 };
                break;
            // Bottom triangle (House 7)
            case 7:
                points.rashiLabelPos = { x: center.x, y: outerDiamond[2].y - labelOffset };
                points.planetCenter = { x: center.x, y: (outerDiamond[2].y + innerDiamond[1].y + innerDiamond[2].y) / 3 - planetCenterOffsetV };
                break;
            // Bottom-right triangle (House 8)
            case 8:
                points.rashiLabelPos = { x: (outerDiamond[0].x + outerDiamond[1].x) / 2 , y: outerDiamond[2].y - labelOffset *0.5 };
                points.planetCenter = { x:  (outerDiamond[0].x + outerDiamond[1].x) / 2, y: (outerDiamond[2].y + innerDiamond[1].y ) / 2 - planetCenterOffsetV };
                break;
            // Right-bottom triangle (House 9)
            case 9:
                points.rashiLabelPos = { x:  outerDiamond[1].x - labelOffset, y: (outerDiamond[2].y+innerDiamond[3].y)/2 };
                points.planetCenter = { x: (outerDiamond[1].x + innerDiamond[1].x) / 2 - planetCenterOffsetH, y: (outerDiamond[1].y + outerDiamond[2].y + innerDiamond[1].y) / 3 };
                break;
           // Right triangle (House 10)
            case 10:
                points.rashiLabelPos = { x: outerDiamond[1].x - labelOffset, y: center.y };
                points.planetCenter = { x: (outerDiamond[1].x + innerDiamond[0].x + innerDiamond[1].x) / 3 - planetCenterOffsetH, y: center.y  };
                break;
            // Right-top triangle (House 11)
            case 11:
                points.rashiLabelPos = { x: outerDiamond[1].x - labelOffset, y: (outerDiamond[0].y +  innerDiamond[0].y) / 2};
                points.planetCenter = { x: (outerDiamond[1].x + innerDiamond[1].x) / 2 - planetCenterOffsetH, y: innerDiamond[0].y };
                break;                
            // Top-right triangle (House 12)
            case 12:
                points.rashiLabelPos = { x: (outerDiamond[0].x + outerDiamond[1].x) / 2 , y:outerDiamond[0].y + labelOffset*0.7  };
                points.planetCenter = { x: ( outerDiamond[1].x + center.x) / 2 + planetCenterOffsetH, y:  (outerDiamond[0].y + innerDiamond[3].y) / 2 };
                break;
            default: // Fallback
                points.planetCenter = this.center;
                points.rashiLabelPos = { x: this.center.x, y: this.center.y - 20 };
        }
        return points;
    }
    ...
    _drawKundaliGrid() {
        const gridGroup = this._createSVGElement('g', { class: 'kundali-grid' });
        ...
        // Draw outer square
        gridGroup.appendChild(this._createSVGElement('polygon', {
        ...
        gridGroup.appendChild(this._createSVGElement('line', {
            x1: outerSquare[0].x, y1: outerSquare[0].y,
            x2: outerSquare[2].x, y2: outerSquare[2].y,
            ...mainStyle
        }));
        gridGroup.appendChild(this._createSVGElement('line', {
            x1: outerSquare[1].x, y1: outerSquare[1].y,
            x2: outerSquare[3].x, y2: outerSquare[3].y,
            ...mainStyle
        }));
        ...
        const midPoints = [
             { x: (outerSquare[0].x + outerSquare[1].x) / 2, y: outerSquare[0].y }, // Top-Mid
             { x: outerSquare[1].x, y: (outerSquare[1].y + outerSquare[2].y) / 2 }, // Right-Mid
             { x: (outerSquare[2].x + outerSquare[3].x) / 2, y: outerSquare[2].y }, // Bottom-Mid
             { x: outerSquare[3].x, y: (outerSquare[3].y + outerSquare[0].y) / 2 }  // Left-Mid
        ];
        gridGroup.appendChild(this._createSVGElement('polygon', {
    ...
    _drawHouseLabels() {
        ...
        const labelGroup = this._createSVGElement('g', { class: 'kundali-house-labels' });
        ...
        for (let houseNum = 1; houseNum <= 12; houseNum++) {
            ...
            textElement.textContent = (1+rashiIndex) + " " +rashiNames[rashiIndex];
    ...
    _drawPlanets(planetsData) {
        if (isNaN(this.ascendantLongitude)) {
            console.error("Cannot draw planets: Ascendant longitude is not set.");
            return;
        }

        const planetGroup = this._createSVGElement('g', { class: 'kundali-planets' });
        ...
        planetsData.forEach(planet => {
            ...
            const houseNum =1+ (12- Math.floor(this.ascendantLongitude/30) + Math.floor(planet.longitude/30)) %12;
            ...
            const baseCoords = this._getHouseAnchorPoints(houseNum).planetCenter;
            housePlanetPositions[houseNum].push({ id: planet.id, baseCoords: baseCoords });
        });
        ...
        for (const houseNum in housePlanetPositions) {
            ...
            planetsInHouse.forEach((planetPos, index) => {
                ...
                group.addEventListener('click', () => {
                    if(this.selectedPlanetId === planetData.id) this.selectPlanet(null);
                ...
                textElement.textContent = planetData.retrograde?planetData.name+"*":planetData.name || '?';
     ...
     * @param {string | null} planetId - The ID of the planet or null to deselect.
    ...
    selectPlanet(planetId) {
        ...
        const selectedPlanetData = this.currentPlanetsData.find(p => p.id === planetId);
        this.selectedPlanetId = planetId;
        const planetGroup = this.svg.querySelector(`.planet-group-${planetId}`);
        if (planetGroup) {
            const highlightCircle = planetGroup.querySelector(`.highlight-circle`);
            if (highlightCircle) {
            ...
            console.warn(`Could not find element for planet "${planetId}" to highlight.`);
        }
     ...
     * @param {Array<object>} planetsData - Array of planet objects [{ id: '...', longitude: ... }].
     * @param {number} ascendantLongitude - The longitude of the Ascendant (Lagna).
     */
