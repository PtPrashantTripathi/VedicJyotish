class GrahaManager {
    constructor() {
        ...
        if (typeof Ephemeris === 'undefined') console.error("Ephemeris library not loaded (needed by PlanetManager).");
        if (typeof getAyanamsa === 'undefined') console.error("panchang.js (getAyanamsa) not loaded or defined globally (needed by PlanetManager).");
        if (typeof daysSinceKaliYugaStart === 'undefined') console.error("panchang.js (daysSinceKaliYugaStart) not loaded or defined globally (needed by PlanetManager).");
    }
     ...
     * @returns {Array<object>} Array of planet data objects.
     */
    getPlanets() {
        return this.planets;
    }
     ...
     * @returns {object | undefined} The planet data object or undefined if not found.
     */
    ...
    /**
     * Calculates planet positions using the Ephemeris library. Updates internal state.
     * @param {Date} date - The date/time for calculation.
     * @param {boolean} [sayan=false] - Whether to calculate Sayana (tropical) positions.
     */
    calculatePlanetPosition(date, sayan = false,location) {
        if (typeof Ephemeris === 'undefined' || typeof getAyanamsa === 'undefined') {
            console.error("Dependencies (Ephemeris/getAyanamsa) not available for calculation.");
            this.planets.forEach(p => { p.longitude = NaN; });
            return;
        }
        ...
        try {
            ...
            const ephemeris = new Ephemeris.default({
                key: Object.keys(this.planetConfig).filter(k => k !== 'rahu' && k !== 'ketu'),
                ...
                latitude: location.lat, longitude: location.lon, height: 0, // Geocentric
            ...
            this.planets.forEach(p => {
                ...
                if (p.id === 'rahu') {
                    ...
                    p.longitude = ephemeris.moon?.orbit?.meanAscendingNode?.apparentLongitude ?? NaN;
                    p.retrograde = true;
                } else if (p.id === 'ketu') {
                    // Ketu is opposite Rahu
                    p.longitude = (ephemeris.moon?.orbit?.meanAscendingNode?.apparentLongitude + 180) ?? NaN;
                    p.retrograde= true;
                } else if(p.id === 'earth') {
                    p.longitude = NaN;
                    p.positionAngle = ephemeris.Earth.position?.polar[0]*180/Math.PI;
                } else {
                    // Other planets
                    const planetResult = results.find(res => res.key === p.id);
                    p.longitude = planetResult?.position?.apparentLongitude ?? NaN;
                    if(p.id != 'sun') p.positionAngle = planetResult?.position?.polar[0]*180/Math.PI;
                    p.retrograde = planetResult?.motion?.isRetrograde;
                    p.sunEarthObjectAngle = Math.acos(-planetResult?.locals.ep)*180/Math.PI;
                    
                    if(p.id == 'moon') {
                        p.illuminatedFraction = planetResult?.position.illuminatedFraction?? 0.5; // k
    ...
    /**
     * Calculates approximate planet positions using Surya Siddhanta mean motions. Updates internal state.
     * @param {Date} date - The date/time for calculation.
     * @param {boolean} [sayan=false] - Whether to calculate Sayana (tropical) positions.
     */
    calculateSuryaSidhdhantaPosition(date, sayan = false) {
        if (typeof daysSinceKaliYugaStart === 'undefined' || typeof getAyanamsa === 'undefined') {
            console.error("Dependencies (daysSinceKaliYugaStart/getAyanamsa) not available for SS calculation.");
            this.planets.forEach(p => { p.longitude = NaN; });
            return;
        }
