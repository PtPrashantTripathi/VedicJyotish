class RashiChakra {
    constructor(svgId) {
        ...
        this.rashiNames = typeof rashiNames !== 'undefined' ? rashiNames : Array(12).fill('?');
        this.nakshatraNames = typeof nakshatraNames !== 'undefined' ? nakshatraNames : Array(27).fill('?');

        // Instantiate ZodiacBackgroundDrawer
        this.backgroundDrawer = new ZodiacBackgroundDrawer(this.svg, this.center, this.center, {
            radii: {
    ...
    _createSunSvg() {
        ...
        // Central body
        sunGroup.appendChild(this._createSVGElement('circle', {
            cx: 0,
            cy: 0,
        ...
        for (let i = 0; i < numRays; i++) {
            ...
            sunGroup.appendChild(this._createSVGElement('line', {
                x1: x1, y1: y1, x2: x2, y2: y2, stroke: sunColor, 'stroke-width': 1.5 }));
        }
        return sunGroup;
    }
    ...
    _drawPlanets(grahaData) {
         ...
         this.svg.querySelectorAll('g[id^="planet-group-"]').forEach(g => g.remove());
         ...
         grahaData.forEach((graha, index) => {
            ...
            const coords = this._getCoordinates(currentPlanetRadius, graha.longitude);
            ...
            if (graha.id === 'sun') {
                ...
                sunIcon.setAttribute('transform', `translate(${coords.x}, ${coords.y})`);
                ...
                })).textContent = graha.retrograde ? graha.name + "*" : graha.name || '?';
            } else {
     ...
     * @param {Array<object>} grahaData - Array of planet objects, each with at least 'id' and 'longitude'.

     */
    drawWheel(grahaData) {
        if (!this.svg) return;
        if (!grahaData || !Array.isArray(grahaData)) {
            console.error("Invalid or missing planetsData passed to drawWheel.");
            return;
        }
    ...
    _drawSelectionLine(planetData) {
        if (!planetData || isNaN(planetData.longitude)) {
            console.warn("Cannot draw selection line for planet with invalid longitude:", planetData);
            return;
        }

        // Calculate end point slightly beyond the outer radius for visibility
        const lineEndCoords = this._getCoordinates(this.outerRadius + 5, planetData.longitude); // Extend slightly
        ...
        const firstPlanetGroup = this.svg.querySelector('g[id^="planet-group-"]');
        if (firstPlanetGroup) {
    ...
    _drawInfoBox(graha) {
        if (!graha || isNaN(graha.longitude)) {
            console.warn("Cannot draw info box for planet with invalid data:", graha);
            return;
        }
        ...
        this.infoBoxGroup.appendChild(this._createSVGElement('rect', {
            x: boxX,
            y: boxY,
    ...
    /**
     * Selects a planet, drawing a line from the center through it and showing an info box,
     * or deselects if planetId is null or invalid.
     * @param {string | null} grahaId - The ID of the planet (e.g., 'sun', 'moon') or null to deselect.
     * @returns {object | null} The selected planet data object or null if deselected/not found.
     */
    selectPlanet(grahaId) {
        ...
        this.svg.querySelectorAll(`textPath.selected-rashi-text`).forEach(tp => {
            tp.classList.remove('selected-rashi-text');
        });
        this.svg.querySelectorAll(`textPath.selected-nakshatra-text`).forEach(tp => {
        ...
        if (planetGroup) {
            ...
            console.warn(`Could not find SVG group for planet ID "${grahaId}" to apply highlight.`);
        }
        ...
        const rashiTextPath = this.svg.querySelector(`textPath[href="#zodiac-rashi-path-${rashiIndex}-${this.svg.id}"]`);
        if (rashiTextPath) {
            rashiTextPath.classList.add('selected-rashi-text');
        }

        const nakshatraTextPath = this.svg.querySelector(`textPath[href="#zodiac-nakshatra-path-${nakshatraIndex}-${this.svg.id}"]`);
